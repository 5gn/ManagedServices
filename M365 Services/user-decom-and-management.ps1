...
function Get-UserAzureAccount {
    ...
    while($confirm -ne 'y') {
        write-host
        #Get the users UPN
        $UserUPN = Read-Host "Please enter in the users full UPN you're wanting to modify"
...
while ($selection -ne "exit" -and $selection -ne "e")
{
    clear
    write-host
    ...
    if ($licenses.count -eq 0) {$menu9 = "[Nill assigned]"} else {$menu9 = "[Licenses assigned]"}
    ...
    if ($groups.count -eq 0) {$menu7 = "[Nill assigned]"} else {$menu7 = "[Groups assigned]"}
    ...
    if ($reports.count -eq 0) {$menu8 = "[Nill assigned]"} else {$menu8 = "Direct reports are assigned]"}
    ...
    write-host "3  - Update account name to include ZZZ_ at the start"
    ...
    write-host "6  - Remove Users Manager"
    write-host "7  - Remove the user from all Groups $menu7"
    write-host "8  - View people that this user is a manager of $menu8"
    write-host "9  - Manage User Licenses $menu9"
    ...
    switch($selection){
       ...
       3 {
            ...
            $error.Clear()
            try {Set-AzureADUser -ObjectID $UserDetails.userprincipalname -DisplayName $NewDisplayName -GivenName $NewFirstName -Surname $NewSurname -Department "Decommisioned" -ErrorAction SilentlyContinue}
       ...
       7 {
            ...
            if (-not $error)
            {
                ...
                write-host
                write-host
                write-host "The user wasn't able to be removed from the following groups:" -ForegroundColor Red
                $groups | Format-table
                write-host
                pause
            }
